...
class Encoder_imp(torch.nn.Module):
    def __init__(self, num_features, dim, num_gc_layers, pooling):
        super(Encoder_imp, self).__init__()
    ...
    def forward(self, x, edge_index, batch, node_imp):
        # max_imp=node_imp.max()
        # min_imp = node_imp.min()

        # mapping node_imp to [0.9,1.1]
        if node_imp is not None:
            out, _ = torch_scatter.scatter_min(torch.reshape(node_imp, (1, -1)), batch)
            out = out.reshape(-1, 1)
            out = out[batch]
            node_imp /= (out*10)
            # max_imp = node_imp.max()
...
class Encoder_defense(torch.nn.Module):
    def __init__(self, num_features, dim, num_gc_layers,device):
        super(Encoder_defense, self).__init__()
    ...
    def forward(self, x, edge_index, batch,defense='none'):
        if x is None:
            x = torch.ones((batch.shape[0], 1)).to(device)
        ...
        for i in range(self.num_gc_layers):
            ...
            if i ==0:
                if defense == 'DP_gauss':
                    noise = DP_noise(1, x.shape, mode='gauss').to(self.device)
                    x = x + noise
                elif defense == 'DP_laplace':
                    noise = DP_noise(1, x.shape, mode='laplace').to(self.device)
                    x = x + noise
                elif defense == 'noise':
                    x = x + torch.randn(x.shape).to(self.device)
                elif defense == 'droup_out':
                    x = F.dropout(x, p=0.8)
                else:
                    print('no defense')
            xs.append(x)
    ...
    def get_embeddings(self, loader,defense='none'):
        ...
        with torch.no_grad():
            for data in loader:
                ...
                if x is None:
                    x = torch.ones((batch.shape[0],1)).to(device)
                x, _ = self.forward(x, edge_index, batch,defense)
...
class Net(torch.nn.Module):
    def __init__(self,num_features,num_classes):
        ...
        self.encoder = Encoder(num_features, dim,3)
